...
const ensureFixedAdmins = async () => {
  const admins = [
    {
      email: process.env.FIXED_ADMIN_EMAIL || 'admin@rzz.de',
      password: process.env.FIXED_ADMIN_PASSWORD || 'admin1234',
  ...
  try {
    ...
    for (const a of admins) {
      const existing = db.prepare('SELECT id, is_admin FROM users WHERE email = ?').get(a.email);
      if (!existing) {
        const hash = await bcrypt.hash(a.password, 10);
        db.prepare(
          `INSERT INTO users (id, email, password, first_name, last_name, is_admin) VALUES (?, ?, ?, ?, ?, 1)`
        ).run(uuidv4(), a.email, hash, a.first_name, a.last_name);
        console.log(`✓ Fixed admin created: ${a.email}`);
      } else if (!existing.is_admin) {
        db.prepare(`UPDATE users SET is_admin = 1 WHERE email = ?`).run(a.email);
        console.log(`✓ Fixed admin promoted: ${a.email}`);
      }
    }
  } catch (err) {
...
const ensureSeedData = () => {
  try {
    ...
    // Lazy import to avoid bcrypt cost unless we seed.
    ...
    const seed = async () => {
      ...
      db.prepare(
        `INSERT INTO users (id, email, password, first_name, last_name)
         VALUES (?, ?, ?, ?, ?)`
      ).run(demoUserId, demoEmail, demoHash, demoFirstName, demoLastName);
